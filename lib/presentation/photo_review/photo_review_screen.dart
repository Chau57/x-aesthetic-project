...
class _PhotoReviewScreenState extends State<PhotoReviewScreen> {
  ...
  Future<void> _showEvaluation({bool force = false}) async {
    try {
      ...
      if (mounted) {
        AppSnack.show(context, 'Không phân tích được ảnh này.');
      }
    }
  }
  ...
  Future<void> _saveToLibrary() async {
    ...
    try {
      ...
      final photo =
          await app.saveCurrentCaptureToLibrary(evaluation: evaluation);
      if (!mounted) {
        return;
      }
      setState(() => _saved = photo != null);
      AppSnack.show(
          context,
          photo == null
              ? 'Không có ảnh để lưu.'
      ...
      if (mounted) {
        AppSnack.show(context, 'Không lưu được ảnh. Vui lòng thử lại.');
      }
    } finally {
      if (mounted) {
...
class _ReviewActions extends StatelessWidget {
  ...
  Widget _buildAnalysisButton(XThemeTokens tokens) {
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color:
                tokens.surface.withValues(alpha: tokens.isDark ? 0.42 : 0.52),
            borderRadius: BorderRadius.circular(999),
