...
Task `1.1 Infrastructure / Software Architecture Skeleton` has been added to the repository. The mobile skeleton now exists under `x_aesthetic_app/` and includes:

* Flutter app bootstrap and placeholder screens for Camera, Preview, and Dashboard.
* Microkernel plugin contract in `x_aesthetic_app/lib/core/plugin/base_plugin.dart`.
* Plugin orchestration via `PluginRegistry` and `PluginManager`.
* Shared models for camera frame, camera pose, detection results, aesthetic attributes, reference styles, and aesthetic results.
* Architecture documentation in `x_aesthetic_app/docs/architecture.md`.
* Plugin contract documentation in `x_aesthetic_app/docs/plugin_contract.md`.
* System Mermaid diagram in `x_aesthetic_app/docs/diagrams/system_architecture.mmd`.
* Smoke test in `x_aesthetic_app/test/core/plugin/base_plugin_test.dart`.

See `docs/task_1_1_output.md` for the handoff note.
